<?php
	...
	if( !class_exists('Wbcr_FactoryTypes000_Type') ) {
		 ...
		 * @since 1.0.0
		 */
		abstract class Wbcr_FactoryTypes000_Type {
			 ...
			 *
			 * @since 1.0.0
			 * @var FactoryViewtables000_Viewtable
			 */
			public $view_table;
			 ...
			 *
			 * @since 1.0.0
			 * @var Wbcr_Factory000_ScriptList
			 */
			public $scripts;
			...
			public function __construct(Wbcr_Factory000_Plugin $plugin)
			{
				...
				$this->menu = new Wbcr_FactoryTypes000_Menu($this);
			...
			public function registerForAdmin()
			{
				...
				foreach($this->metaboxes as $metabox) {
					Wbcr_FactoryMetaboxes000::registerFor($metabox, $this->name, $this->plugin);
				}
				...
				if( $this->template !== 'public' ) {
					//Wbcr_FactoryMetaboxes000::registerFor('Wbcr_FactoryMetaboxes000_PublishMetabox', $this->name);
			...
			private function buildLables()
			{
				...
				$labels = array(
					...
					'name' => $plural_name,
					'all_items' => sprintf(__('All %1$s', 'wbcr_factory_types_000'), $plural_name),
					'add_new' => sprintf(__('Add %1$s', 'wbcr_factory_types_000'), $singular_name),
					'add_new_item' => sprintf(__('Add new', 'wbcr_factory_types_000'), $singular_name),
					'edit' => sprintf(__('Edit', 'wbcr_factory_types_000')),
					'edit_item' => sprintf(__('Edit %1$s', 'wbcr_factory_types_000'), $singular_name),
					'new_item' => sprintf(__('New %1$s', 'wbcr_factory_types_000'), $singular_name),
					'view' => sprintf(__('View', 'wbcr_factory_types_000')),
					'view_item' => sprintf(__('View %1$s', 'wbcr_factory_types_000'), $singular_name),
					'search_items' => sprintf(__('Search %1$s', 'wbcr_factory_types_000'), $plural_name),
					'not_found' => sprintf(__('No %1$s found', 'wbcr_factory_types_000'), $plural_name),
					'not_found_in_trash' => sprintf(__('No %1$s found in trash', 'wbcr_factory_types_000'), $plural_name),
					'parent' => sprintf(__('Parent %1$s', 'wbcr_factory_types_000'), $plural_name)
				);
			...
			public function actionUpdatedMessages($messages)
			{
				...
				$replacements = array(
					...
					array(
						'{scheduled}',
						date_i18n(__('M j, Y @ G:i', 'wbcr_factory_types_000'), strtotime($post->post_date))
					)
				);
